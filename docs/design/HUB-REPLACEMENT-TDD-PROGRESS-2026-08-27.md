...
27 commits (`e60c4c4`..`4b337f1`), all independently verified by the terminal (full `pytest -q` + `pyright` after every commit, not just cx's own report — see "Verification discipline" below). Test suite: **1079 passed**, 1 known pre-existing unrelated failure (`test_generate_manifest.py`'s committed hub.py hash snapshot, stale before this session started) + occasional pre-existing flakiness in one real multi-threaded CAS-race test under full-suite load (`test_two_real_callers_race_at_attempt_creation_one_loses_cleanly`, passes cleanly in isolation, unrelated to any change this session).

**All 5 real domains now have working CLI wiring**: `peerhub consensus propose|vote|status`, `peerhub task create|claim-start|checkpoint|complete|fail|cancel|status`, `peerhub lesson propose|approve|activate|retire|supersede|quarantine|status`, `peerhub room create|create-thread|append-message|clear|status`, `peerhub duty claim|heartbeat|close|status` — all real, runnable commands with `--json` output, not just Python/test-callable code.

**`LegacyTranslator` (`peerhub/application/legacy.py`) now backs 11 of ~90 `LEGACY_CATALOG` actions** (up from 3 at session start): `ask`/`ask-all`/`ask-coordinator` (pre-existing) + `consensus-propose`/`consensus-vote`/`consensus-check` + `task-checkpoint`/`task-status`/`task-failover` + `lessons-propose`/`lessons-activate`/`lessons-retire` + `new-topic`/`clear-room` — every legacy action for every domain this session built a native service for. **Important, confirmed distinction**: this is a genuinely SEPARATE subsystem from the CLI wiring above -- `LegacyTranslator` only translates a legacy action name+args into a typed wire `Command` object; it was investigated directly and confirmed that no downstream dispatcher yet exists to actually execute a translated `Command` against a real service (except `ask`/`ask-all`/`ask-coordinator`'s own separate pre-existing path). Every legacy-translation commit this session is honestly scoped as translation-only for this reason -- finding/building that execution dispatcher is real, separate, not-yet-started follow-up work.
...
| **gap-1** compat/cutover | Native CLI complete for 5 domains; 11/~90 legacy actions translate (wire-layer only, no execution dispatcher yet) | `peerhub consensus\|task\|lesson\|room\|duty` CLI; `peerhub/application/legacy.py`'s `LegacyTranslator` for 11 actions |

## What's NOT done yet (real, not hypothetical, gaps)

- **gap-1**: ~79 of ~90 `LEGACY_CATALOG` actions still untranslated (everything outside consensus/task/lesson/room/duty's covered subset — session/messaging/governance-proposal/health/routing actions etc.). More importantly: **no execution dispatcher exists connecting a translated `Command` to a real service call** for any of the 11 newly-backed actions — this needs to be found (if it exists somewhere unexplored) or built before legacy callers could actually run end-to-end through this path, not just have their action names successfully translate.
