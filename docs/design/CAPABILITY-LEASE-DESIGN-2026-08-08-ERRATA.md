...
**Outcome: no real adapter currently has invocation-correlated DIR-004 enforcement evidence; no production implementation is warranted.** This source audit establishes what each adapter and launcher know, but source inspection is not proof that a requested control took effect. It therefore cannot itself be promoted to `cli_live`, `app_server`, `statusline`, or `empirical_probe` evidence.

- `RealAgyAdapter.plan_invocation()` constructs only `agy.exe -p <prompt> --output-format json`, with an empty environment delta and no sandbox/confinement control ([agy_adapter.py:121–148](file:///P:/peerhub/peerhub/adapters/agy_adapter.py#L121-L148)). This agrees with the existing empirical finding that ag has no enforceable filesystem confinement. Its ceiling remains absent, not `CONFINED`.
- `RealClaudeAdapter.plan_invocation()` constructs only `claude.cmd -p <prompt> --output-format json`, also with an empty environment delta ([claude_adapter.py:128–155](file:///P:/peerhub/peerhub/adapters/claude_adapter.py#L128-L155)). It reads no permission configuration and receives no launcher observation proving a restriction. Its enforcement ceiling remains `TEST NEEDED`/absent.
- `RealCodexAdapter.plan_invocation()` constructs only `codex.cmd exec --json <prompt>`, again with an empty environment delta ([codex_adapter.py:139–166](file:///P:/peerhub/peerhub/adapters/codex_adapter.py#L139-L166)). It supplies no `--sandbox`/`-s` value, reads no Codex configuration, and cannot know whether this invocation realized `read-only`, `workspace-write`, or another sandbox. Even adding such a flag would initially prove only command construction, not enforcement. Its ceiling remains `TEST NEEDED`/absent.

There is no hidden corroboration later in the current path. `InvocationPlan` carries argv, cwd, environment, transport, stdin, limits, display text, artifacts, and session action—but no enforcement observation ([adapters/contract.py:432–451](file:///P:/peerhub/peerhub/adapters/contract.py#L432-L451)). The pipe launcher passes those values to `subprocess.Popen()` and records process identity/tree supervision facts ([pipe.py:280–318](file:///P:/peerhub/peerhub/dispatch/pipe.py#L280-L318)); that process-tree mechanism supports cancellation and is not a filesystem, Git, or remote-mutation confinement proof. `ProcessTerminalEvidence` contains only exit/signal/timing/cancellation facts ([execution.py:30–56](file:///P:/peerhub/peerhub/core/execution.py#L30-L56)). The direct bootstrap's `cli_live` evidence is an executable-readiness observation ([bootstrap.py:119–138](file:///P:/peerhub/peerhub/application/bootstrap.py#L119-L138)), not sandbox evidence, so it must not be reused as an enforcement source tag.

The current ordering also prevents a literal post-spawn observation from supporting the existing pre-intent receipt: the receipt is constructed immediately after planning ([workflows.py:581–598](file:///P:/peerhub/peerhub/application/workflows.py#L581-L598)), dispatch intent commits at [workflows.py:707–725](file:///P:/peerhub/peerhub/application/workflows.py#L707-L725), and only then does process spawn begin at [workflows.py:747](file:///P:/peerhub/peerhub/application/workflows.py#L747). A future implementation must either prepare and attest an OS-enforced execution context before intent and then spawn into that already-bound context, or ratify a different state/spawn protocol. Merely inspecting a process after it has started cannot retroactively justify the pre-spawn receipt and may be too late to prevent a prohibited mutation.

Accordingly, the shipped provider must continue returning `enforcement_ceiling=None` and `source_tag="absent"` for ag/cc/cx ([capability_policy.py:77–90](file:///P:/peerhub/peerhub/dispatch/capability_policy.py#L77-L90)), and `dispatch_and_execute()` must retain the honest `"unverified"` receipt fields ([workflows.py:588–598](file:///P:/peerhub/peerhub/application/workflows.py#L588-L598)). Mutating tiers remain fail-closed at the pre-plan gate; `READ_ONLY` remains explicitly `ADVISORY`, not enforced.

Before any future adapter replaces those placeholders, all of the following are required:

1. a machine-owned launcher/evidence provider must prepare and attest the realized control for the exact invocation before the process can execute, rather than echoing adapter argv;
2. that observation must be bound to a canonical digest of the exact materialized plan/process identity and carry one of the DIR-004 source tags;
3. an empirical negative probe must demonstrate that the claimed boundary actually blocks the prohibited mutation for that adapter/profile/control combination; and
4. the post-plan gate must compare the receipt to that independent observation before dispatch intent commits. The current revalidation hook checks the authoritative capability binding and policy revision when a receipt is supplied, but does not yet corroborate `controls_description`, `evidence_source_tag`, or `plan_digest` ([attempt_lifecycle.py:175–234](file:///P:/peerhub/peerhub/dispatch/attempt_lifecycle.py#L175-L234)).

Until those prerequisites exist, changing any real adapter's receipt to a positive enforcement claim would fabricate evidence and violate DIR-004.
