...
CREATE TABLE shim_pending_operations (
    ...
    -- Populated only by the force_complete_unconfirmed_durability operator intervention
    durability_override_reason TEXT,
    durability_override_at TEXT,
...
2. **INTENT_DECLARED → FS_STAGED**
   * *FS (staging only, no replacement of `P` yet)*: For operations writing a payload (`intended_registry_outcome='ACTIVE'`, `RESTORE`), derive a deterministic staging path based purely on the operation's identity: e.g. `staging_path = P.peerhub-stage.<sha256(idempotency_key)>` (using the full 64-hex-character digest to eliminate collision risk). This ensures a resumed process always computes the exact same path.
     ...
     * *Backup creation* (if `operation_kind='EXTERNAL_COLLISION'`): Derive backup artifact paths similarly (e.g. `P.peerhub-backup.<sha256(idempotency_key)>.bak` and its `.json` meta). Give the finalized `.bak` path its own explicit inspect-before-act sequence:
       ...
       5. *Backup DB Commit*: `BEGIN IMMEDIATE`; `INSERT ... ON CONFLICT (originating_idempotency_key) DO NOTHING` into `shim_backup_entries` (generating `backup_sequence_id`); `COMMIT`. This ensures the `.bak` is durably tracked as a legitimate, quarantinable resource immediately after creation, before proceeding to main payload staging.
...
* **`force_complete_unconfirmed_durability(idempotency_key, reason)`** — For the specific case where an operation is permanently stuck in `FS_STAGED` because its filesystem write (`os.replace`/`os.remove`) succeeded but the required subsequent `fsync` fails persistently (e.g., unresolvable permission/storage condition on the file or parent directory).
  ...
  2. Run the EXACT SAME read-only determination logic Step 3 / `abandon_stuck_operation` use. If the determination is anything OTHER than "already applied" (i.e. "never applied" or "external modification"), REJECT with a typed error directing the operator to `abandon_stuck_operation` instead — this operation is not for those cases.
  3. If "already applied": attempt the required fsync(s) (file + parent directory, and for `RESTORE`, the metadata reapplication + its own fsync) exactly once more. If it succeeds, complete normally via the standard already-applied completion path — no override was actually needed, and the `reason` is simply persisted as an audit note explaining why the operator intervened.
  4. If the fsync attempt fails again: proceed to DB completion anyway (same completion steps: registry status update, `RESTORE` backup `restored=1` if applicable, `operation_state='COMPLETED'`), but explicitly record that durability was NOT confirmed. Set the new `durability_override_reason` to the operator's reason and `durability_override_at` to the current time on the `shim_pending_operations` row. (This is an explicit, disclosed, operator-acknowledged risk where content is verified correct but the durability guarantee against a future crash is unconfirmed. Normal completion path durability guarantees remain completely unchanged.)
  5. Release the lock.

Combined, `abandon_stuck_operation`, `mark_backup_permanently_unusable`, and `force_complete_unconfirmed_durability` give the operator a full recovery path.
