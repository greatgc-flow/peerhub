...
* **`force_reconcile_stuck_operation(idempotency_key, expected_current_hash)`** — For the specific case where an operation is permanently stuck in `FS_STAGED` due to a degenerate missing-stage scenario (e.g., a `RESTORE` of identical bytes that must re-derive from an archive that is now permanently unreadable). This generalizes the exact pattern already established by `accept_current_as_baseline`.
  1. Acquire the same advisory lock used throughout this state machine, scoped to the operation's registration (resolved prior to lock acquisition, as noted above).
  2. `BEGIN IMMEDIATE`; re-validate that the `idempotency_key` still maps to the acquired registration. Require `operation_state='FS_STAGED'` for the referenced operation (read-check, reject if not in this state).
  3. Re-verify `sha256(P) == expected_current_hash` live (the operator must have independently confirmed this using tools/access OUTSIDE this design's own potentially-blocked verification path -- e.g., direct filesystem access, a different tool, manual inspection). Abort if it doesn't match (do not trust a stale claim).
  4. Compare `expected_current_hash` against the operation's own ALREADY-PERSISTED `pre_state_hash`/`post_state_hash` (both plain DB reads -- this comparison needs NO access to the archive, backup, or stage):
     - **If it equals `post_state_hash`** (the effect already happened): complete the operation normally through the SAME DB-only completion logic step 3's own "already applied" branch already uses (mark the backup restored if `RESTORE`, set the registry's final status, mark `operation_state='COMPLETED'`). No filesystem write is needed or attempted here, since `P` already has the correct content by the operator's own independently-verified confirmation.
     - **If it matches NEITHER `pre_state_hash` NOR `post_state_hash`** (genuine external modification): this is a safely-determined outcome equivalent to the standard external-modification abort. Explicitly CAS the operation to `ABORTED` applying the normal retirement logic in this specific sub-case.
     - **If it equals `pre_state_hash`** (the effect never happened): this is the one case this operation CANNOT safely resolve -- writing the correct payload requires actual source data (the archive/stage) that is by definition inaccessible in this scenario, and this design must never synthesize or guess content. In this specific sub-case, `force_reconcile_stuck_operation` must reject with a clear, honestly-worded error explaining that the payload genuinely cannot be written without access to its source, and that resolution requires either (a) restoring access to the archive/backup so normal automatic retry can succeed, or (b) accepting this as a residual, disclosed limitation of the design requiring manual intervention outside its scope. Do NOT attempt to CAS this sub-case to `ABORTED`, as forcing an abort without confirming `P`'s true relationship to the operation could leave the registry's recorded state wrong relative to reality.
  5. Release the lock.

Combined, `force_reconcile_stuck_operation` and `mark_backup_permanently_unusable` give the operator a full (if occasionally honest-limitation-bounded) recovery path: try `force_reconcile_stuck_operation` first with an independently-verified hash (resolves "already applied" and "external modification" cases); if that itself reports the payload is genuinely unwritable without archive access, the honest answer is that this specific operation requires either environment repair or is a disclosed residual limitation -- not a silent unsafe workaround.
