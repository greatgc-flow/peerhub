...
- **Mechanism**: an OS-level advisory file lock (NOT a plain "lock file exists = locked" convention, which would need its own stale-lock/timeout handling) -- specifically, an exclusively-locked file descriptor held open for the duration of the operation, using the platform's native advisory locking primitive (POSIX: `fcntl.flock(fd, LOCK_EX)`; Windows: `msvcrt.locking(fd, msvcrt.LK_LOCK, ...)` or an equivalent OS-level exclusive lock). This is the load-bearing property: OS-level advisory locks are tied to the OPEN FILE DESCRIPTOR's lifetime, not to any application-level state, so a crashed or killed process automatically and immediately releases the lock when its file descriptors close -- eliminating stale-lock/timeout concerns entirely by construction, rather than requiring the design to detect and recover from a stale lock.
- **Lock file path**: derived deterministically from `shim_registration_id` (e.g., a dedicated `.peerhub-lock.<shim_registration_id>` file in the isolated archive/lock directory established by the A2 security fix, NOT adjacent to `P` or inside the PATH-exposed shim directory -- matching the same PATH-exposure discipline already established for backup archives).
- **Scope**: exactly one lock per registration, matching every existing reference to "the advisory lock scoped to a specific registration" throughout the document.
...
* **`resolve_effect_state(P, op, *, force_apply_degenerate_restore=False) -> (outcome, was_degenerate)`** — Pure, read-only, no DB transaction, no filesystem write. Computes `actual_hash = sha256(P)` (or notes absence) and returns one of `NEVER_APPLIED`, `ALREADY_APPLIED`, `EXTERNALLY_MODIFIED`, plus whether the degenerate branch was taken (`was_degenerate`, needed by `finalize_operation` below for one RESTORE-specific rule). Defined only when `P` is absent or a regular file with `st_nlink == 1` (per the document-wide Safe File Operations discipline); any other live state (symlink, directory, device node, or hardlinked file) is rejected before this primitive returns anything, at whichever call site is inspecting `P` at the time.
