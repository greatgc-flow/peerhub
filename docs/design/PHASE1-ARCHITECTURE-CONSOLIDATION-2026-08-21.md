...
2. **INTENT_DECLARED → FS_STAGED**
   * *FS (staging only, no replacement of `P` yet)*: For operations writing a payload (`intended_registry_outcome='ACTIVE'`, `RESTORE`), derive a deterministic staging path based purely on the operation's identity: e.g. `staging_path = P.peerhub-stage.<sha256(idempotency_key)>` (using the full 64-hex-character digest to eliminate collision risk). This ensures a resumed process always computes the exact same path.
     ...
     * *Backup creation* (if `operation_kind='EXTERNAL_COLLISION'`): Derive backup artifact paths similarly (e.g. `P.peerhub-backup.<sha256(idempotency_key)>.bak` and its `.json` meta). Give the finalized `.bak` path its own explicit inspect-before-act sequence:
       ...
       4. Independently validate or atomically regenerate `backup_meta.json` alongside whichever path was taken, strictly applying the same safe file operation discipline, and requiring it and its parent directory to be `fsync`ed. Regenerate it using the `original_mtime_epoch`/`original_permissions_octal` ALREADY persisted in this operation's own `shim_pending_operations` row (captured live from `P` at Step 1, before this operation ever began) -- NEVER by re-reading `P`'s current live state here, since `P`'s metadata could have drifted (e.g. an external `touch`) between Step 1 and this resumed attempt, and the operation's own already-committed intent row is the only value that's authoritative for what `P` looked like when this operation started. If this sidecar generation fails terminally AFTER the `.bak` was already finalized+fsynced but BEFORE the sidecar is durable (and therefore before the Backup DB Commit), the failure-handling path MUST explicitly delete the already-finalized `.bak` (using no-follow unlink) before transitioning to `ABORTED` -- since no DB row was ever committed for it, it cannot be left behind untracked.
...
* **`force_complete_unconfirmed_finalization(idempotency_key, reason)`** — For the specific case where an operation is permanently stuck in `FS_STAGED` because its filesystem write (`os.replace`/`os.remove`) succeeded (so the content is verifiably correct) but a required subsequent finalization step fails persistently (e.g., unresolvable permission/storage condition causing `fsync` to fail, or metadata reapplication failing for a `RESTORE`).
  ...
  2. Run the EXACT SAME read-only determination logic Step 3 / `abandon_stuck_operation` use. This operation still only ever admits the "already applied" determination (a genuine content/byte mismatch, degenerate-case metadata tie-break resolving to "never applied", or "external modification" must never be converted into a forced success). If the determination is anything OTHER than "already applied" (i.e. "never applied" or "external modification"), REJECT with a typed error directing the operator to `abandon_stuck_operation` instead — this operation is not for those cases.
  3. If "already applied": attempt BOTH remaining finalization steps exactly once more: for `RESTORE`, the metadata reapplication itself, AND the required fsyncs (file + parent directory, and the metadata fsync). If they succeed, complete normally via the standard already-applied completion path — no override was actually needed, and the `reason` is simply persisted as an audit note explaining why the operator intervened.
