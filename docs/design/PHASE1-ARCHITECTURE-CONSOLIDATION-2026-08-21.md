...
* **`mark_backup_permanently_unusable(backup_sequence_id, reason)`** — Automatic escalation from "repeated operational failure" to "quarantined" is explicitly out of scope for this persistence design (as it requires policy decisions like retry-count thresholds that belong to a higher operational layer). However, if a human operator diagnoses a backup as genuinely, permanently broken (e.g., after observing repeated persistent ACL denials or sharing lock violations on the same archive across multiple retry attempts), they can use this explicit escalation path. The operation simply executes `UPDATE shim_backup_entries SET corrupt_detected_at=... WHERE backup_sequence_id=?`, permanently quarantining it so no future `RESTORE` will select it and starve older valid backups. The `reason` string serves purely as audit/log context for the operator's decision.
