...
3. **FS_STAGED → COMPLETED | ABORTED**
   * *FS (uniform pre-effect re-validation / TOCTOU narrowing)*, performed immediately before touching `P`, identically on every execution path (first attempt and any crash resumption): compute `actual_hash = sha256(P)` (or note absence). Evaluate as a strictly ordered decision procedure:
     1. **Degenerate case (`pre_state_hash == post_state_hash`)** (a no-op reinstall of identical bytes): The FIRST discriminant is always `actual_hash == pre_state_hash`. If this holds, we distinguish "genuinely unchanged" from "coincidentally-degenerate but tampered" (or "already applied" vs "never applied"). If it does not hold, fall through to bullet 4: unconditional external modification, with NO metadata-based override. Split by operation family:
        * For ACTIVE-outcome operations (`ABSENT`/`EXTERNAL_COLLISION`/`MANAGED_UPDATE`): skip any filesystem write, **but explicitly `fsync` `P` itself (using the same safely-opened, no-follow discipline) and its parent directory to guarantee durability**. If this required `fsync` (or parent-directory fsync) fails for any reason (including platform-specific access-rights limitations, such as Windows `FlushFileBuffers` requiring a `GENERIC_WRITE`-capable handle on an externally-authored file), the operation must NOT silently proceed to `COMPLETED`. Instead, it must abort as a transient/retryable condition (explicitly leaving the operation in its current `FS_STAGED` state and safely releasing the lock), so a later retry attempt can re-attempt the fsync once the underlying condition (e.g. a permission or handle issue) is resolved. Only if the `fsync` succeeds does it proceed directly to completion (this is the only legitimate outcome in this branch).
        * For `RESTORE`, resolve the tie by checking `P`'s current `mtime`/permissions against the backup's REAL PERSISTED `original_mtime_epoch`/`original_permissions_octal` from `shim_backup_entries`—matching, treat as "already applied" (following bullet 3's completion logic); not matching, treat as "never applied" (following bullet 2's write logic). If "never applied" but the deterministic stage file is missing (e.g. consumed by a prior `os.replace`), explicitly re-derive/re-stage from the still-intact `.bak` archive rather than treating a missing stage file as an error. **During this re-derivation, apply the exact same two-tier permanent vs. transient validation checks to the `.bak` archive as in Step 2: structural failures must auto-quarantine and transition to `ABORTED`, while operational failures must abort transiently (leaving state in `FS_STAGED`).** Otherwise, verify its hash matches, re-stage it, and proceed to write.
     2. **Normal case, `actual_hash == pre_state_hash`** (or `P` genuinely still absent when `pre_state_hash IS NULL`) → the effect has **never been applied**. Safe to apply it now:
        ...
        * For `EXTERNAL_COLLISION` specifically: before proceeding with the filesystem replacement, look up whether this operation has an associated backup (via `originating_idempotency_key` matching this operation's own `idempotency_key`) with `corrupt_detected_at IS NOT NULL`. If such a quarantined backup exists, do NOT proceed with the write. Instead, treat this identically to the "external modification" branch's handling: inside a `BEGIN IMMEDIATE` transaction, CAS the operation to `ABORTED` (verifying affected rows equal 1), apply the retirement logic, and `COMMIT`; release the advisory lock; explicitly delete the deterministically-named stage if it exists (using no-follow unlink); raise `ERR_ORIGINATING_BACKUP_QUARANTINED`. A quarantined backup means safety for this specific collision write can no longer be guaranteed, since the operator has already determined the backup (the only recovery path for the foreign content this write would destroy) is unusable. ("Already applied" as determined by `mark_backup_permanently_unusable` at one point in time does NOT bind the collision operation's own later, independent resume — `P` remains externally mutable in between, so this explicit check at write-time is what actually closes the gap, not merely the earlier point-in-time determination.) If the backup is NOT already quarantined, perform a just-in-time archive re-validation immediately before writing. Reuse the EXACT same two-tier structural-vs-operational validation pattern Step 2 already establishes for RESTORE's own `.bak` validation:
          1. Safely inspect (no-follow, reject non-regular file, reject hardlinked per the established `st_nlink == 1` discipline) the operation's own originating `.bak` archive.
          2. Verify its hash matches `pre_state_hash` (the content this backup was supposed to preserve). Also re-`fsync` it and its parent directory to refresh durability confidence before proceeding.
          3. **Structural failure** (missing, non-regular file type, hardlinked, or hash mismatch): immediately auto-quarantine -- in the SAME transaction as the CAS-abort, ALSO set `corrupt_detected_at`/`corrupt_reason` on the backup row (mirroring Step 2's own structural-failure auto-quarantine pattern), CAS the operation to `ABORTED`, apply retirement logic, `COMMIT`, clean up the stage, and raise a typed error (reuse `ERR_ORIGINATING_BACKUP_QUARANTINED`, since the practical effect -- an unusable backup blocking this destructive write -- is identical whether the corruption was previously recorded or just discovered).
          4. **Operational failure** (permission/lock/IO error reading or fsyncing the archive): treat as transient/retryable -- abort this specific attempt WITHOUT writing and WITHOUT any CAS transition, leaving `operation_state` exactly at `FS_STAGED` (matching the established transient-vs-structural pattern used everywhere else in this document), release the advisory lock, so a later retry can re-attempt once the underlying condition resolves.
          5. Only if the archive re-validation passes does the write proceed exactly as in Round 142's logic.
   ...
   * *DB (on success only)*: `BEGIN IMMEDIATE`;
     ...
     * If `intended_registry_outcome='RETIRED'`, `UPDATE shim_registry_entries SET status='RETIRED', updated_at=? WHERE shim_registration_id=?` instead.
...
* If the `shim_registry_entries` row's CURRENT status is `PROVISIONING` (meaning this operation was a fresh install that never completed), it is atomically updated to `RETIRED` (updating `updated_at=...` as well) in the same transaction that marks the operation `ABORTED` (or rolled back entirely if step 1 hadn't committed). This frees the `shim_name` and `canonical_shim_path` for a fresh attempt by a new operation.
...
* **`mark_backup_permanently_unusable(backup_sequence_id, reason)`** — Automatic escalation from "repeated operational failure" to "quarantined" is explicitly out of scope for this persistence design (as it requires policy decisions like retry-count thresholds that belong to a higher operational layer). If a human operator diagnoses a backup as genuinely, permanently broken (e.g., after observing repeated persistent ACL denials or sharing lock violations on the same archive across multiple retry attempts), they can use this explicit escalation path. **This operation is lock-guarded and coordination-aware**, since a plain unconditional update would deadlock against an already-in-flight `RESTORE` that previously selected this exact backup (backup selection is pinned once at `INTENT_DECLARED` and never re-queried, so an unresolved stuck operation would remain pinned to the now-corrupt backup forever while also blocking any fresh `RESTORE` attempt via `idx_active_pending_operation`). Furthermore, a backup's own originating operation is a distinct, previously-unchecked coordination participant from any operation that later selects it for restoration, and both must be accounted for before quarantine can safely proceed:
  ...
  4. **Exactly one in-flight operation found at `INTENT_DECLARED`**: safe to resolve automatically — `INTENT_DECLARED` guarantees `P` has not been replaced, NOT that no filesystem write occurred at all (step 2 can have already created and `fsync`ed a deterministic stage before crashing ahead of the `FS_STAGED` DB commit). In the SAME transaction: `UPDATE shim_backup_entries SET corrupt_detected_at=..., corrupt_reason=? WHERE backup_sequence_id=? AND corrupt_detected_at IS NULL` (verifying affected-row-count equals 1), AND `UPDATE shim_pending_operations SET operation_state='ABORTED' WHERE idempotency_key=? AND operation_state='INTENT_DECLARED'` (verifying affected rows equal 1), applying the existing retirement logic (an `EXTERNAL_COLLISION` operation always has a `PROVISIONING` parent registry row per Step 1, so aborting it retires that row, freeing the name for a fresh attempt; a `RESTORE` leaves the registry `ACTIVE`). `COMMIT`. Explicitly delete any stale deterministic stage that may already exist for the operation being quarantined (using safe no-follow unlink).
  5. **Exactly one in-flight operation found at `FS_STAGED`**:
     - If the operation is a `RESTORE` (found via `selected_backup_sequence_id`): NOT safe to resolve automatically — the filesystem effect may or may not have already happened. Reject the quarantine request with a typed error (e.g., `ERR_BACKUP_IN_USE_BY_ACTIVE_RESTORE`) instructing the operator to first resolve the in-flight operation (via normal resume, `abandon_stuck_operation`, or `force_complete_unconfirmed_finalization`, as appropriate) before retrying quarantine. Never blindly abort an `FS_STAGED` operation from this path. Explicitly roll back/close the transaction before releasing the advisory lock, not just return an error.
     - If the operation is the backup's own ORIGINATING `EXTERNAL_COLLISION` (found via `originating_idempotency_key`): Use the exact read-only determination from Step 3 / `abandon_stuck_operation` to safely inspect `P` before proceeding, all under the same lock and in the same transaction as the quarantine decision. Branch on the determination:
       - "already applied" (`P == post_state_hash`, meaning the destructive overwrite has ALREADY happened): the backup can no longer protect anything additional by blocking, and quarantining it has no unsafe interaction with the collision operation's own eventual completion. The collision operation's own later, independent resume will re-evaluate its CURRENT state at THAT time and take whichever outcome is then correct -- complete without writing (if truly still "already applied" at that point), abort on external modification, or (per the Round 142/144 guards) abort if its backup is found quarantined or its archive fails re-validation. No single deterministic outcome is guaranteed by this operation's own point-in-time observation -- that is exactly why leaving the collision operation's own state untouched (rather than trying to force a particular outcome here) is both correct and sufficient, since the operation's own resume logic now correctly handles every possible state it might find itself in. Do NOT attempt to complete the collision operation from within this operation -- doing so would require performing its FS re-fsync work inside this same transaction, violating the "no transaction held open across a filesystem write" invariant. Simply quarantine the backup via the exact same plain `UPDATE shim_backup_entries SET corrupt_detected_at=..., corrupt_reason=? WHERE backup_sequence_id=? AND corrupt_detected_at IS NULL` used by the "No in-flight operation found" case (affected-row-count checked), `COMMIT`, leaving the collision operation's own state completely untouched.
       - "never applied" or "external modification" (`P` still matches `pre_state_hash`, or has been externally modified since -- the original foreign content, or some other content, is NOT yet overwritten by this collision operation): quarantine the backup AND CAS-abort the collision operation atomically in the SAME transaction (mirroring the existing `INTENT_DECLARED` branch's safe-abort pattern), applying the retirement logic (a `PROVISIONING` row is retired), with affected-row counts verified. `COMMIT`; only AFTER commit, explicitly delete the deterministically-named stage if it exists (using no-follow unlink); release the advisory lock. This is what actually prevents the destructive write the operator was trying to avoid by quarantining in the first place.
  6. **BOTH in-flight checks find a non-terminal operation simultaneously**: This should be structurally unreachable under the invariants above; if it is ever observed, treat it as evidence of database corruption or a violated invariant, and fail closed by rejecting rather than attempting to resolve two operations at once. Explicitly roll back/close the transaction before releasing the advisory lock, not just return an error.
  7. Release the advisory lock at the end of either the success or rejection path.
...
* **`abandon_stuck_operation(idempotency_key)`** — For the specific case where an operation is permanently stuck in `FS_STAGED` due to a degenerate missing-stage scenario (e.g., a `RESTORE` of identical bytes that must re-derive from an archive that is now permanently unreadable). Unlike the removed `force_reconcile_stuck_operation`, this operation requires no operator-supplied hash because nothing about determining state was ever unreliable in this scenario -- only the ACT OF WRITING (when required) was blocked, and this operation never attempts that write; it only completes when the effect provably already happened, or safely cancels otherwise.
  ...
  2. Outside any transaction (while holding the lock, matching how normal step 3 already hashes `P` before its own `BEGIN IMMEDIATE`), require `operation_state='FS_STAGED'` for the referenced operation (read-check, reject if not in this state), then run EXACTLY the same read-only determination step 3 already performs on every resumption:
     ...
     - If `pre_state_hash == post_state_hash` (the degenerate case) and `operation_kind='RESTORE'`:
       - If `actual_hash != pre_state_hash`: treat as "external modification".
       - Else if `P`'s live `mtime`/permissions match the backup's persisted `original_mtime_epoch`/`original_permissions_octal` from `shim_backup_entries`: treat as "already applied".
       - Else: treat as "never applied" (this is the case that could not resolve automatically because the archive access needed to re-derive/rewrite was permanently blocked -- it's exactly what `abandon_stuck_operation` exists to safely cancel).
     - Otherwise (all other cases, including degenerate ACTIVE-outcome operations): `actual_hash == post_state_hash` → "already applied"; `actual_hash == pre_state_hash` → "never applied"; anything else → "external modification".
...
* **`force_complete_unconfirmed_finalization(idempotency_key, reason)`** — For the specific case where an operation is permanently stuck in `FS_STAGED` because its filesystem write (`os.replace`/`os.remove`) succeeded (so the content is verifiably correct) but a required subsequent finalization step fails persistently (e.g., unresolvable permission/storage condition causing `fsync` to fail, or metadata reapplication failing for a `RESTORE`).
  1. Require a non-empty `reason` string (reject if blank/null). Acquire the same advisory lock used throughout this state machine, scoped to the operation's registration (resolved via a clean autocommit read before lock acquisition). Require `operation_state='FS_STAGED'`.
  2. Run the read-only determination logic Step 3 / `abandon_stuck_operation` use, but with one explicit, deliberately scoped admission-gate exception for this operation only: for a degenerate `RESTORE` (`pre_state_hash == post_state_hash`) where the file's bytes are objectively correct (`actual_hash == pre_state_hash`), treat this as "already applied" for this operation's own purposes REGARDLESS of what the metadata tie-break would otherwise conclude (since unrecoverable metadata preventing completion is exactly what this operation exists to bypass). Any genuine byte mismatch (`actual_hash` outside `{pre_state_hash}`, i.e. "external modification") must still be rejected. This exception applies ONLY within `force_complete_unconfirmed_finalization` -- normal Step 3 resume and `abandon_stuck_operation` are UNCHANGED and continue to use the metadata tie-break as their determination, which remains necessary for their own purposes (normal resume needs to know whether a write is still required, and `abandon_stuck_operation`'s safe-cancel guarantee depends on "never applied" meaning `P` genuinely still holds pre-operation content). If the determination (after applying this narrow exception) is anything OTHER than "already applied" (i.e. "never applied" or "external modification"), REJECT with a typed error directing the operator to `abandon_stuck_operation` instead — this operation is not for those cases.
  3. If "already applied": attempt BOTH remaining finalization steps exactly once more: for `RESTORE`, the metadata reapplication itself, AND the required fsyncs (file + parent directory, and the metadata fsync). If they succeed, complete normally via the standard already-applied completion path WITHOUT setting `finalization_override_reason` or `finalization_override_at` at all (leave both `NULL`, exactly like any other ordinary completion) — no override was actually needed.
  4. If EITHER step fails again (metadata reapplication itself, or the fsync after it succeeds): proceed to DB completion anyway (same completion steps: registry status update, `RESTORE` backup `restored=1` if applicable, `operation_state='COMPLETED'`), but explicitly record that finalization (durability confirmation and/or RESTORE metadata) was NOT confirmed. Set the new `finalization_override_reason` to the operator's reason and `finalization_override_at` to the current time on the `shim_pending_operations` row. (This is an explicit, disclosed, operator-acknowledged risk where content correctness is verified, but secondary non-safety-critical finalization steps are unconfirmed. Normal completion path guarantees remain completely unchanged.)
  5. Release the lock.
