...
**Operator recovery from `ABORTED` and permanently corrupted backups (Fixes F1 & F5):**
...
* **`mark_backup_permanently_unusable(backup_sequence_id, reason)`** — Automatic escalation from "repeated operational failure" to "quarantined" is explicitly out of scope for this persistence design (as it requires policy decisions like retry-count thresholds that belong to a higher operational layer). If a human operator diagnoses a backup as genuinely, permanently broken (e.g., after observing repeated persistent ACL denials or sharing lock violations on the same archive across multiple retry attempts), they can use this explicit escalation path. **This operation is lock-guarded and coordination-aware**, since a plain unconditional update would deadlock against an already-in-flight `RESTORE` that previously selected this exact backup (backup selection is pinned once at `INTENT_DECLARED` and never re-queried, so an unresolved stuck operation would remain pinned to the now-corrupt backup forever while also blocking any fresh `RESTORE` attempt via `idx_active_pending_operation`):
  1. Acquire the same advisory lock used throughout this state machine, scoped to the registration this backup belongs to.
  2. `BEGIN IMMEDIATE`; check for any non-terminal (`operation_state NOT IN ('COMPLETED', 'ABORTED')`) row in `shim_pending_operations` with `selected_backup_sequence_id` equal to the target backup.
  3. **No in-flight operation found**: directly `UPDATE shim_backup_entries SET corrupt_detected_at=..., corrupt_reason=? WHERE backup_sequence_id=?`; `COMMIT`.
  4. **In-flight operation found at `INTENT_DECLARED`**: safe to resolve automatically — per the Round 104-106 universal ordering (`FS_STAGED` is committed to the database *before* the final filesystem replacement of `P`, for every operation kind), an operation still at `INTENT_DECLARED` is guaranteed to have made no filesystem write yet. In the SAME transaction: `UPDATE shim_backup_entries SET corrupt_detected_at=..., corrupt_reason=? WHERE backup_sequence_id=?`, AND `UPDATE shim_pending_operations SET operation_state='ABORTED' WHERE idempotency_key=? AND operation_state='INTENT_DECLARED'` (affected rows must equal 1), applying the existing retirement logic (the registry stays `ACTIVE`, matching the established rule that a `RESTORE`/`MANAGED_UPDATE`/`REMOVE` abort against an already-established shim leaves the registry untouched); `COMMIT`.
  5. **In-flight operation found at `FS_STAGED`**: NOT safe to resolve automatically — the filesystem effect may or may not have already happened, and only that operation's own step-3 pre-effect re-validation (run on its normal resume path) can correctly determine which. Reject the quarantine request with a typed error (e.g. `ERR_BACKUP_IN_USE_BY_ACTIVE_RESTORE`) instructing the caller to first resolve the in-flight `RESTORE` through its own normal resume path before retrying quarantine. Never blindly abort an `FS_STAGED` operation from this path.
