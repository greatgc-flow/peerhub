...
- **Health/quota tracking (T2)** - `HEALTH-QUOTA-TRACKING-DESIGN-2026-08-16.md`. **Canary PASSED 2026-08-17** (`acb6635`). **Increment 1 DONE (`e41d22b`)**: usage/readiness projection persistence layer (migration `0024`, mirrors the existing proven `ReadinessObserved` pattern, `quota_pool_scope` correctly included in the PK so concurrent pools don't clobber -- 972 passed, pyright clean). **Remaining**: the actual `TelemetryWorker` background-polling logic (CLI shell-outs per Section 2.1-2.3, including the `stdin=DEVNULL` requirement) and the `peerhub status` CLI surface (Section 4) -- not started.
