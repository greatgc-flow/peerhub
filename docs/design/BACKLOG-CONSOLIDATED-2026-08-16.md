...
- **Health/quota tracking (T2)** - `HEALTH-QUOTA-TRACKING-DESIGN-2026-08-16.md`. **Canary PASSED 2026-08-17** (`acb6635`) -- Codex/agy worked as designed; Claude's initial timeout was root-caused to a real, permanent peerhub characteristic (non-TTY subprocess context adds a ~3s stdin-wait claude.exe doesn't hit under hub.py's TTY-attached context) and fixed in the design (`stdin=subprocess.DEVNULL` + >=15s deadline, required in the real implementation). No longer gated -- ready to implement. Migration `0024_telemetry_quota_tracking` (was `0023` in the original doc; EvidenceArtifact landed first and took `0023`, doc updated).
