...
- **Health/quota tracking (T2)** - `HEALTH-QUOTA-TRACKING-DESIGN-2026-08-16.md`. **Canary PASSED 2026-08-17** (`acb6635`). **Increment 1 DONE (`e41d22b`)**: usage/readiness projection persistence layer (migration `0024`). **Increment 2 DONE (`947e899`)**: `peerhub/telemetry/quota_polling.py`'s `poll_claude_usage()`, applying the `stdin=DEVNULL`+15s fix, real-CLI integration test passed (15.26s) -- 975 passed, pyright clean. **Remaining**: Codex + Agy polling functions, wiring the poll output into the increment-1 write path (`add_usage_observation`), the actual background `TelemetryWorker` scheduling loop, and the `peerhub status` CLI surface (Section 4) -- none started.
