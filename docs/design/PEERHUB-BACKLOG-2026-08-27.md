...
24 of the 90 `LEGACY_CATALOG` actions translate and execute end-to-end, across 6 real domains with full CLI + native service + legacy-translation + execution-dispatcher coverage: consensus, task, lesson, room (partial), duty-lease, room-participation-session. Full detail: `docs/design/HUB-REPLACEMENT-TDD-PROGRESS-2026-08-27.md`. 66 actions remain unbacked. This doc inventories all of them plus the older, pre-TDD-phase backlog items from the README that never got folded into the TDD tracking.
...
| Legacy action | Native method | Real backing already exists |
|---|---|---|
| `terminal-close` (`--close-session` half) | `coordination.terminal.close` (extend) | `RoomParticipationCoordinator.end_session()` — the seam was deliberately left open in the `terminal-close` commit specifically for this; now unblocked since sessions exist. |
| `terminal-duty-sweep` | `coordination.terminal.duty_sweep` | Borderline -- the per-lease primitive exists (`DutyLeaseCoordinator.expire_and_recover_lease(lease_id, ...)`, built for gap-4) but it operates on ONE known `lease_id`, not a bulk scan. A "sweep" needs a new small orchestration loop (enumerate ACTIVE `terminal-duty` leases past `heartbeat_expires_at`, call `expire_and_recover_lease` on each) -- real but thin new code, not zero-new-code wiring. Verify the no-auto-replay semantics match the gap-3 doc's description before building it. |
| `thread-append` | `coordination.thread.append` | `RoomsService.append_message()` — already built and used internally by other tests; just never exposed through `LegacyTranslator`/CLI. |
| `lesson-broadcast` | `coordination.lesson.broadcast` | `LessonService.record_delivery_pending(lesson_id, peer_id, *, delivery_method="broadcast", ...)` — the `delivery_method` parameter already defaults to exactly this concept. Confirm the legacy semantics match "mark delivery pending for N peers" before wiring, don't assume blindly. |
...
|---|---|---|
| `thread-react` | Immutable append-only reaction events + a maintained "current reaction" projection (not a mutable CAS status flip) | The reaction event schema/table (or `TargetState` family, TBD at build time) and the coordinator/service methods. |
