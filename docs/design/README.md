...
- `engram-sys-folder-rename-feasibility-2026-09-17.md` — **SUPERSEDED then REVERSED (2026-09-18)**: initially concluded dynamic `_sys` renameability wasn't worth attempting; user pushed back on the Python import-namespace "boundary," and a `sys.modules`-bootstrap technique (empirically verified by cc with a real test run) closes it with zero changes to the 9 affected files' import statements. Revised recommendation: viable as two small, independently-verified steps (import bootstrap + the path-centralization work from Addendum 1), pending cx.
