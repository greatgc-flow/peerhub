...
* **Fixtures:**
  * **Positive (`fix-end-session-pos-01`, NYI):** Pre-state: `state.json` contains `cc`, `handoff.md` exists. Request: `end-session cc`. Expected exit: 0. Output: `[END] cc 세션 종료 완료`. Post-state: `cc` removed from `state.json`, `handoff.md` appended, EXIT logged.
...
* **Specific Argv Comparators:** Cwd/Env independent.
* **Fixtures:**
  * **Positive (`fix-mark-read-pos-01`, NYI):** Pre-state: Mailbox has unread msg ID 1 for `cx`. Request: `mark-read cx --msg-id 1`. Expected exit: 0. Output: `[READ] 1개 메시지 읽음 처리`. Post-state: Msg ID 1 status is `read`, unread count decremented.
  * **Invalid (`fix-mark-read-inv-01`, NYI):** Pre-state: Mailbox empty. Request: `mark-read cx --msg-id 999`. Expected exit: 0. Output: `[READ] 0개 메시지 읽음 처리`. Post-state: Unchanged.
