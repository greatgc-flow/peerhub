...
Per DIR-004, every entry carries a source tag. `empirical_probe` means it was
measured on a real invocation; a fact that was only ever read out of a doc or
a `--help` blurb is `declared, unverified` and says so.
