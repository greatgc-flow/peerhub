...
The PTY premise in the test draft is especially stale: the empirical probe at `docs/history/design/phase0/PTY-BUFFERING-PROBE-2026-08-03.md` records plain pipes for `cc`, `ag`, and `cx`, with `DEVNULL`/closed stdin required for `ag` and `cx`. The production runner implements that mitigation and supports timestamped chunks, silence timeouts, cancellation, and process-tree control. The draft should build on that, not redesign it from its outdated premise.
