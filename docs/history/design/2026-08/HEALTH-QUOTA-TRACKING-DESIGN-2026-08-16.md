...
**Architectural gap and resolution (implemented 2026-08-19)**: this section originally did not specify WHAT PROCESS actually hosts the "background" worker. `peerhub` today has no persistent daemon/server concept at all. 

**Decision & Implementation Status**: 
1. **Poll-on-demand (COMPLETED/SHIPPED)**: `peerhub status --peer/--all` and `peerhub diag --fresh` now directly trigger a fresh poll (via `_refresh_usage_projections` in `cli.py`) with their own TTL-based caching check against the existing projection's `updated_at`. This provides accurate, fresh telemetry without needing a new process model.
2. **Ambient background daemon (DEFERRED)**: A genuinely new `peerhub daemon` or `peerhub serve` subcommand that runs a persistent process to poll in the background remains deferred. The poll-on-demand implementation satisfies the immediate requirements.
