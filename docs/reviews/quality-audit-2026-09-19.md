...
**RESOLVED (cc, 2026-09-20):** decomposed into 5 private methods, each independently callable and testable, matching the function's own already-present `# Step N` comment boundaries exactly (a purely mechanical extraction -- no control-flow or behavior changes):
- `_plan_and_gate_invocation` -- capability gate, session-capability check, `plan_invocation()`, enforcement receipt.
- `_prepare_and_materialize_artifacts` -- attempt creation, manifest generation/recording, materialization; returns an early `ExecutionWorkflowResult` on materialization failure.
- `_reserve_dispatch_intent` -- the dispatch-intent/artifact-reservation try/except block; returns an early `ExecutionWorkflowResult` on reservation failure.
- `_spawn_and_supervise_process` -- process spawn, heartbeat worker, the `_on_spawned`/`_on_process_chunk` closures; returns an early `ExecutionWorkflowResult` on spawn failure or start-uncertain.
- `_assess_and_terminalize` -- output interpretation/decoding, completion assessment, and terminalization (or leave-for-recovery if lease ownership was lost).

`dispatch_and_execute` itself is now a ~90-line orchestrator that calls the 5 phases in sequence, checking each intermediate result for an early-exit `ExecutionWorkflowResult` -- the exact same short-circuit control flow as before, just visible as `isinstance` checks between phase calls instead of buried in 462 lines of nested logic. 4 new frozen dataclasses (`_InvocationPlanResult`, `_ArtifactPreparationResult`, `_ReservationResult`, `_ProcessSupervisionResult`) carry each phase's output to the next, replacing what were previously ~10 loose local variables threaded through nested closures.

**Verification:** pyright clean on the first pass (all extracted parameter/return types lined up correctly). All 46 pre-existing tests in `tests/unit/application/test_workflows_dispatch_and_execute.py` and `test_workflows_dispatch_with_retries.py` -- which already cover every one of these branches end-to-end, including `test_dispatch_and_execute_materialization_failure`, `test_dispatch_and_execute_reservation_failure`, and `test_dispatch_and_execute_rejects_session_without_capability` -- pass unchanged, with no test modifications required. Full suite: 1730 passed, 0 regressions. No new phase-isolated unit tests were added on top of this, since the existing 46 tests already exercise every phase and branch via the public entry point; the decomposition's value is that a *future* test targeting one phase's edge case no longer needs to construct all 16 of `dispatch_and_execute`'s parameters to do it.
