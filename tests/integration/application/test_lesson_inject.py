...
def test_legacy_lesson_inject_translates_and_executes(tmp_path):
    ...
    from peerhub.application.legacy import LessonInjectCommand
    ...
    with create_runtime(context, adapter_peer_kind="fake") as runtime:
        # Add a lesson through the real service (same as before)
        _add_lesson(
        ...
        # Submit a LessonInjectCommand through the real ApplicationAPI
        caller = RequestContext(principal="test-user", client_id="test-client")
        ...
        assert outcome.ok
        assert outcome.result["injection_block"] is not None
        assert "legacy-test" in outcome.result["injection_block"]
        assert "legacy rule" in outcome.result["injection_block"]
