...
def _seed_dctx_credential(workspace: Path, *, credential_id: str, peer_instance_id: str) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        issue_credential(
            ...
            issued_at=0,
            # The real CLI's credential_verifier uses SystemClock.now() (real
            # wall-clock epoch seconds), not this test's FixedClock -- a
            # small sentinel like 999999999 (year 2001) would already be
            # "expired" against any real current timestamp.
            expires_at=4102444800,
        )
        conn.commit()


def _setup_verified_round(tmp_path: Path) -> int:
    """Create a workspace with cc/cx as eligible voters and a real
    --verified-required proposal round via the CLI. Callers read the round
    ID from the CLI's captured stdout (see test_cli_add_and_vote_exact_
    compatibility_stdout for the same pattern); returns the seed timestamp."""
...
def test_cli_proposal_verified_required_accepts_real_credential(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_cli_proposal_verified_required_rejects_impersonation(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A credential PeerHub minted for peer cx must not authorize a
    proposal-vote cast as a different peer (cc), even through the
    legacy-compatible proposal-vote CLI path."""
...
def test_cli_proposal_verified_required_rejects_missing_credential(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    _setup_verified_round(tmp_path)
