...
def test_ss_01_first_ever_dispatch():
    ...
    policy = SessionPolicy(mode="fresh", soft_pressure_threshold=75, hard_pressure_threshold=90, max_observation_age_ms=30000)
...
def test_ss_02_auto_between_soft_and_hard():
    ...
    policy = SessionPolicy(mode="auto", soft_pressure_threshold=75, hard_pressure_threshold=90, max_observation_age_ms=30000)
    result = saga.evaluate_and_claim(
        session_policy=policy,
        workspace_scope_id="scope",
        instance_id="inst",
...
def test_ss_06_auto_stale_evidence():
    ...
    policy = SessionPolicy(mode="auto", soft_pressure_threshold=75, hard_pressure_threshold=90, max_observation_age_ms=1)
    result = saga.evaluate_and_claim(
        session_policy=policy,
        workspace_scope_id="scope",
        instance_id="inst",
...
def test_ss_08_reuse_pressure_above_hard():
    ...
    saga = SessionRotationSaga(FakeDispatchRepo(), FakeTelemetryRepo(FakeProjection(95, 100)), FakeClock(), FakeIdSource())
    policy = SessionPolicy(mode="reuse", soft_pressure_threshold=75, hard_pressure_threshold=90, max_observation_age_ms=30000)
    result = saga.evaluate_and_claim(
        session_policy=policy,
        workspace_scope_id="scope",
        instance_id="inst",
