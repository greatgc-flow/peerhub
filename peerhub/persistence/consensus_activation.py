...
def read_activation_state(connection: sqlite3.Connection) -> str:
    triggers_query = connection.execute(
        "SELECT name FROM sqlite_master WHERE type='trigger' AND name IN ('consensus_v2_guard_insert', 'consensus_v2_guard_update', 'consensus_v2_guard_delete')"
    ).fetchall()
    trigger_names = {t[0] if type(t) is tuple else t["name"] for t in triggers_query}
    has_all_triggers = len(trigger_names) == 3
