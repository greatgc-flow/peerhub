"""Service layer for session binding, lease management, and recovery orchestration."""
...
class DispatchUnitOfWork(UnitOfWork, Protocol):
    ...
    def cas_update_lease(
        ...
        """CAS-update a lease if its revision is current."""
...
class DispatchService:
    """Orchestrate scoped session-binding, lease, and recovery operations."""
        ...
        with self._store.unit_of_work() as unit:
            ...
            if binding is None:
                raise RecordNotFoundError(
                    "session_binding",
                    f"{request.key.instance_id}/{request.key.profile_id}",
                )
            ...
            if not is_compatible:
                if not unit.cas_update_session_binding(binding, updated):
        ...
        *,
        ...
        with self._store.unit_of_work() as unit:
            ...
            if not unit.cas_update_lease(current, updated):
                ...
                raise InvalidMutationError(
                    f"CAS failure renewing lease {request.lease_id} "
                    f"(expected rev {current.fence.revision}, found "
    ...
    def close_lease(self, request: LeaseCloseRequest) -> LeaseSnapshot:
    ...
    def check_lease_fence(
        ...
        with self._store.unit_of_work() as unit:
            current = unit.get_lease(request.requester_fence.lease_id)
            if current is None:
    ...
    def recover_lease(
        ...
        *,
        ...
        with self._store.unit_of_work() as unit:
            ...
            if not unit.cas_update_lease(current, updated_lease):
                raise InvalidMutationError(
